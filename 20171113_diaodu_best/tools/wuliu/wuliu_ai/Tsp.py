...
def tsp_greedy(orderList, riderId, dataSaver):
    # 对tsp路径进行优化
    riderX = dataSaver.riderFrame[riderId]['mcx']
    riderY = dataSaver.riderFrame[riderId]['mcy']
    # 进行取餐路径优化, 计算骑士，商户matrix
    startPoint = (float(riderX), float(riderY))
    pointList = []
    shopNum = len(orderList)
    for i in range(shopNum):
        orderId = orderList[i]
        orderDic = dataSaver.fenpeiDic[orderId]
        shopX = orderDic['shopMcx']
        shopY = orderDic['shopMcy']
        pointList.append((float(shopX), float(shopY)))
    # 计算路径
    riderShopRout = get_route(startPoint, pointList)
    riderShop = [orderList[thisIndex] for thisIndex in riderShopRout]
    shopAt = orderList[riderShopRout[-1]]
    # 计算商户到用户的matrix
    shopX = dataSaver.fenpeiDic[shopAt]['shopMcx']
    shopY = dataSaver.fenpeiDic[shopAt]['shopMcy']
    startPoint = (float(shopX), float(shopY))
    pointList = []
    userNum = len(orderList)
    for i in range(userNum):
        orderId = orderList[i]
        orderDic = dataSaver.fenpeiDic[orderId]
        userX = orderDic['userMcx']
        userY = orderDic['userMcy']
        pointList.append((float(userX), float(userY)))
    shopUserRoute = get_route(startPoint, pointList)
    shopUser = [orderList[thisIndex] for thisIndex in shopUserRoute]
    return riderShop, shopUser


def distL2(loc1, loc2):
...
def nearest_neighbor(n, i, D):
    ...
    tour = [i]
    while unvisited != []:
        next_item = nearest(last, unvisited, D)
        tour.append(next_item)
...
def tsp_route(point_list):
    n, D = mk_matrix(point_list, distL2)
    ...
    for i in range(n):
        ...
        routes.append([tour, z])
    return routes

def get_route(start, point_list):
    ret = tsp_route(point_list)
    r_map = {}
    dis_arr = []
    for r in ret:
        tour = r[0]
        tour_start = point_list[tour[0]]
        dis = distL2(start, tour_start)
        r_map[dis+r[1]] = tour
        dis_arr.append(dis+r[1])
        # print dis+r[1], tour
    dis_arr.sort()
    return r_map[dis_arr[0]]

def get_route_se(start, end, point_list):
    ret = tsp_route(point_list)
    r_map = {}
    dis_arr = []
    for r in ret:
        tour = r[0]
        tour_start = point_list[tour[0]]
        tour_end = point_list[tour[-1]]
        dis = distL2(start, tour_start)
        dis += distL2(end, tour_end)
        r_map[dis+r[1]] = tour
        dis_arr.append(dis+r[1])
        # print dis+r[1], tour
    dis_arr.sort()
