...
def get_free_rider_num(dataSaver):
    ...
    freeNum = (useFrame.status == 'leisure').sum()
    return freeNum
...
if __name__ == '__main__':
    ...
    while True:
        ...
        # 计算商圈压力，并设置对应的压单量
        setNum, willOrderNum = set_produce_order_num(orderSer, producer)
        ...
        if freeRiderNum > 200:
            # 检查是否有骑士可以变成超级骑士
            dataSaver.check_super_rider(operRecorder)
        print(dataSaver.superDic)
        # 检查是否有预约单可以进入派单流程
        ...
        print(orderNum)
        if orderNum > 0:
            freeRiderNum = get_free_rider_num(dataSaver)
            # 是否进行订单合并
            similarSet = []
            if setNum > 0:
