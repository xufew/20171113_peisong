...
import Config
from .wuliu_ai import SuperRider
...
class DataSaver():
    ...
    def check_super_rider(self, operRecorder):
        '''
        查看骑士是否可以变为super rider
        '''
        if len(self.yuyueDic) != 0:
            chooseDic = SuperRider.choose_rider(
                    self.yuyueDic,
                    self.riderFrame,
                    self.timer,
                    self.time,
            ...
            if len(chooseDic) != 0:
                # 派给对应骑士预约单
                for riderId in chooseDic:
                    orderId = chooseDic[riderId]['orderId']
                    riderX = self.riderFrame[riderId]['mcx']
                    riderY = self.riderFrame[riderId]['mcy']
                    operRecorder.write_info(riderId, '', orderId, riderX, riderY, self.time, '0')
                    atShopTime = chooseDic[riderId]['atShopTime']
                    shopX = self.yuyueDic[orderId]['shopMcx']
                    shopY = self.yuyueDic[orderId]['shopMcy']
                    operRecorder.write_info(riderId, '', orderId, shopX, shopY, atShopTime, '1')
                    # 加入超级骑士名单
                    self.superDic[riderId] = chooseDic[riderId].copy()
                    # 订单从预约单中取出
                    orderDic = self.yuyueDic[orderId].copy()
                    self.superOrderDic[orderId] = orderDic
                    self.yuyueDic.pop(orderId)
                    # 超级骑士状态改变
                    self.riderFrame[riderId]['mcx'] = shopX
                    self.riderFrame[riderId]['mcy'] = shopY
