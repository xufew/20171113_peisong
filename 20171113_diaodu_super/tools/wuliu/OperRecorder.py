...
class OperRecorder():
    ...
    def update_riderFrame(self, dataSaver):
        ...
        for riderId in riderIdList:
            ...
            con3 = hasOrderNum == 0
            if con1 and con3:
                # 如果骑士一直接不到订单，看是否需要完成超级单
                if riderId in dataSaver.superDic:
                    self.go_finish_super(riderId, dataSaver)
            if con1 and con2:
            ...
            if superOrder:
                ...
                if not immediateDeliver:
                    ...
                    if expectTime>finishTime:
                        delt = (expectTime-finishTime).seconds
                        if delt >= Config.yuding_wait:
                            finishTime = timer.add_second_datetime(
                                    expectTime, -Config.yuding_wait
                                    )
                desX = userX
                desY = userY
                desAoi = userAoi
                finishTime = timer.trans_datetime_to_unix(finishTime)
                self.write_info(riderId, '', orderId, userX, userY, finishTime, '3')
                dataSaver.riderFrame[riderId]['routeDic'] = {'shopRoute':{}, 'userRoute':{}}
    ...
    def go_finish_super(self, riderId, dataSaver):
        '''
        完成身上所挂的预订单
        '''
        orderId = dataSaver.superDic[riderId]['orderId']
        yuyueExpectTime = dataSaver.timer.trans_unix_to_datetime(
                dataSaver.superDic[riderId]['expectTime']
                )
        nowTime = dataSaver.timer.trans_unix_to_datetime(
                dataSaver.time
                )
        con1 = nowTime > yuyueExpectTime
        con2 = False
        if yuyueExpectTime >= nowTime:
            con2 = (yuyueExpectTime-nowTime).seconds < Config.super_go_give
        if con1 or con2:
            orderDic = dataSaver.superOrderDic[orderId].copy()
            dataSaver.superOrderDic.pop(orderId)
            dataSaver.orderDic[orderId] = orderDic
            dataSaver.orderDic[orderId]['superDic'] = True
            dataSaver.dispatch_order(orderId, riderId)
            dataSaver.superDic.pop(riderId)
