...
def get_score_order_rider(
    ...
    exactScore, timeScore, waitShop = exact_finish(
    ...
    # 等餐时间越短越好
    outScore += Config.score_wait_shop*waitShop
    return outScore
...
def exact_finish(
    ...
    if atShopTime >= waitShopTime:
        ...
        waitShop = (waitShopTime-atShopTime).seconds/300
        if waitShop > 1:
            waitShop = 1
    ...
    timeScore = 1-totalTime/60.0/120.0
    if timeScore < -1:
        timeScore = -1
    return exactScore, timeScore, waitShop
